 ...
 *      Body (text):       { title, description, optionType:'text',
 *                           option1, option2,
 *                           creatorMint, tiebreak: 1|2, paymentTxSig }
 *      Body (nomination): { title, description, optionType:'nomination',
 *                           nominee, nomineeLink,
 *                           creatorMint, tiebreak: 1|2, paymentTxSig }
 *                           tiebreak 1 = NO, 2 = YES
...
function validateRaiseBody(body) {
  ...
  if (optionType === 'text') {
    ...
    // Nomination = single nominee, yes/no vote. We map internally so option1="NO" (slider left)
    // and option2="YES" (slider right). This keeps every tally / tiebreak / winner code path
    // identical across both types — only labels differ.
    const nominee = String(body?.nominee || '').trim();
    if (!nominee) throw new Error('Nominee is required.');
    if (nominee.length > OPT_TEXT_MAX) throw new Error(`Nominee name too long (${OPT_TEXT_MAX} max).`);
    const link = String(body?.nomineeLink || '').trim();
    if (!link) throw new Error('Nominee profile link is required.');
    let u;
    try { u = new URL(link); } catch { throw new Error('Nominee profile link is not a valid URL.'); }
    const h = u.hostname.toLowerCase();
    if (!/(^|\.)twitter\.com$/.test(h) && !/(^|\.)x\.com$/.test(h))
      throw new Error('Nominee link must be a twitter.com or x.com profile URL.');
    out.nominee     = nominee;
    out.nomineeLink = link;
    out.option1     = 'NO';
    out.option2     = 'YES';
  }
  ...
  if (tiebreak !== 1 && tiebreak !== 2)
    throw new Error('Your tiebreaker vote is required (Option 1 or 2 for text, YES or NO for nominations).');
...
async function hydrateIssue(issue, { viewerWallet = null, viewerMints = null } = {}) {
  ...
  const payload = {
    ...
    nomineeLink:  issue.nomineeLink || null,
  ...
  try {
    ...
    if (action === 'raise') {
      ...
      const issue = {
        ...
        nomineeLink:   body.nomineeLink || null,
